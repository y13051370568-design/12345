...
CREATE TABLE `ml_workflow` (
  ...
  `category` VARCHAR(50) DEFAULT NULL COMMENT '工作流分类',
  `tags` VARCHAR(255) DEFAULT NULL COMMENT '工作流标签，多个标签可用逗号分隔',
  `view_count` INT NOT NULL DEFAULT 0 COMMENT '浏览次数',
  `fork_count` INT NOT NULL DEFAULT 0 COMMENT 'Fork次数',
  `is_recommended` TINYINT NOT NULL DEFAULT 0 COMMENT '是否推荐: 0(否), 1(是)',
  `rejection_reason` VARCHAR(255) DEFAULT NULL COMMENT '审核驳回原因',
  `is_public` TINYINT NOT NULL DEFAULT 0 COMMENT '是否公开: 0(私有), 1(公开)',
  `audit_status` VARCHAR(20) NOT NULL DEFAULT 'PENDING' COMMENT '审核状态: PENDING, APPROVED, REJECTED, OFF_SHELF',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_workflow_audit_public` (`audit_status`, `is_public`),
  KEY `idx_workflow_owner` (`user_id`),
